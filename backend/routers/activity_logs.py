...
def list_activity_logs(
    ...
    try:
        ...
        if action and action != "All Actions":
            if action in ("Predictive Restock", "Predictive"):
                where.append("al.action IN (%s, %s)")
                params.extend(["Predictive Restock", "Predictive"])
            else:
...
def list_highlight_activity_logs(
    ...
    Includes:
    - Login / Logout
    - Predictive Restock actions
    - Inventory create/update/delete where description starts with:
      "Added inventory item%", "Deleted inventory item%", "Updated inventory item%"
    """
    conn = get_db()
    cur = conn.cursor(dictionary=True)
